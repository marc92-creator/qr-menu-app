...
export function QRCodeTab({ restaurant, categories = [] }: QRCodeTabProps) {
  ...
  // Generate PDF using Puppeteer API (professional quality)
  const handleDownloadMenuPDF = async () => {
    setIsGeneratingPDF(true);
    try {
      const response = await fetch(`/api/pdf/menu?slug=${restaurant.slug}&type=menu`);
      if (!response.ok) {
      ...
      link.href = url;
      link.download = `speisekarte-${restaurant.slug}.pdf`;
  ...
  // Generate table tent PDF using Puppeteer API
  const handleDownloadPDF = async (format: PDFFormat) => {
    setIsGeneratingPDF(true);
    setShowFormatModal(false);
    try {
      const response = await fetch(`/api/pdf/menu?slug=${restaurant.slug}&type=tent&format=${format}`);
      if (!response.ok) {
        throw new Error('PDF generation failed');
      }
      const blob = await response.blob();
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `tischaufsteller-${restaurant.slug}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {categories.length > 0 && (
              <button
                ...
                className="flex flex-col items-center gap-2 p-4 bg-gray-50 hover:bg-gray-100 rounded-xl border border-gray-200 transition-all group col-span-2 disabled:opacity-50 disabled:cursor-wait"
              >
                <div className="w-10 h-14 bg-white rounded-lg border border-gray-300 flex items-center justify-center shadow-sm group-hover:shadow-md transition-shadow">
                  {isGeneratingPDF ? (
                ...
                <span className="text-sm font-medium text-gray-700">
                  {isGeneratingPDF ? 'Wird erstellt...' : 'Komplette Speisekarte'}
                </span>
                <span className="text-xs text-gray-500">
                  {isGeneratingPDF ? 'Bitte warten' : 'PDF mit allen Gerichten, Preisen & Allergenen'}
                </span>
              </button>
            )}
          </div>
